...
extension NounCreator {
  ...
  struct MferTraitTypePicker: View {
    @ObservedObject var viewModel: MferViewModel
    let animation: Namespace.ID
    
    @State private var selectedTraitType: Int = MferSlotMachine.TraitType.headphones.rawValue
      ...
      PlainCell(
        ...
        VStack(spacing: 0) {
          VStack(spacing: 0) {
            ...
            MferTraitTypeSegmentedControl(viewModel: viewModel, selectedTraitType: $selectedTraitType)
          }
          .gesture(dragGesture())
          
          // Expand or Fold the collection of Noun's Traits.
          if viewModel.isExpanded {
            MferTraitPickerUIKitView(viewModel: viewModel)
              .frame(height: 250)
              .padding(.vertical, 8)
          }
        }
