...
struct MferTraitPickerUIKitView: UIViewRepresentable {
  typealias UIViewType = MferTraitPickerUIKit
  
  @ObservedObject var viewModel: NounCreator.MferViewModel
  
  private var cancellables = Set<AnyCancellable>()
  
  init(viewModel: NounCreator.MferViewModel) {
    self.viewModel = viewModel
  }
  
  func makeUIView(context: Context) -> MferTraitPickerUIKit {
    let grid = MferTraitPickerUIKit(viewModel: viewModel)
    return grid
  }
  
  func updateUIView(_ uiView: MferTraitPickerUIKit, context: Context) {
    // Left intentionally blank
  }
}

class MferTraitPickerUIKit: UIView {
  
  @ObservedObject var viewModel: NounCreator.MferViewModel
  
  private var cancellables = Set<AnyCancellable>()
  
  private let traitTypes = MferSlotMachine.TraitType.allCases
...
extension MferTraitPickerUIKit: UICollectionViewDelegate, UICollectionViewDataSource {
